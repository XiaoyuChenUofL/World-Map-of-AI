...
#     (df_hit_count["hit"] / k).sum() / n_users
    print(n_users)
    return (df_hit_count["hit"] / k)
...
#     (df_hit_count["hit"] / df_hit_count["actual"]).sum() / n_users
    return (df_hit_count["hit"] / df_hit_count["actual"])
...
def ndcg_at_k(
    ...
    df_dcg = df_dcg.groupby(col_user, as_index=False, sort=False)[col_user].agg({"dcg": "sum"})
...
#     (df_merge["rr"] / df_merge["actual"]).sum() / n_users
    return (df_merge["rr"] / df_merge["actual"])
